...
public class TransactionDAOImpl implements TransactionDAO {
	...
	@Override
	public List<Transaction> getTransactionReport(List<String> dates) throws ParseException {
		List<Transaction> transactionList = null;
		
		    DateFormat inputFormat = new SimpleDateFormat("yyyy-MM-dd");
		    DateFormat outputFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss-SSSS");  

		    Date fromDate = outputFormat.parse(dates.get(0));      
		    Date toDate = outputFormat.parse(dates.get(1));

		    Query query = new Query();

		    Criteria c = new Criteria().andOperator(Criteria.where("updateTime").gte(outputFormat.format(fromDate)),  
		            Criteria.where("updateTime").lte(outputFormat.format(toDate)));

		    query.addCriteria(c);

		    return this.mongoOperations.find(query, Transaction.class);
	...
	public List<Transaction> getTransactionReport(String reportType, String duration) {
      ...
      // Date fromDate = outputFormat.parse(duration);      
       //Date toDate = outputFormat.parse(todate);
         ...
         Criteria c = new Criteria().andOperator(Criteria.where("paidOn").gte("2018-05-02"),  
        Criteria.where("paidOn").lte("2017-05-02"));

        query.addCriteria(c);

        this.template.find(query, Transaction.class);
		/*if(duration!=null){
			...
			transactionList = template.find(query, Transaction.class);
				...
				Query query = new Query(Criteria.where("reportType").is(transaction.getReportType()));
